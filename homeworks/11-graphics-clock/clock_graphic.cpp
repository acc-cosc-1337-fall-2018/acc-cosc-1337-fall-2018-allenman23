...
void ClockGraphic::draw_hours_hand()
{
	...
	double hours_increment = clock.get_minutes() / 60.0 * 30.0;
	hours_angle += hours_increment;
...
{
	std::unique_ptr<Shape> digtime = std::make_unique<Text>(device_context, clock.get_time(), Point(110, 90));
	digtime->draw();
}
